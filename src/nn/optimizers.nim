...
method update*(self: var Optimizer, weights: var Matrix[NNFloat],
    gradients: var Matrix[NNFloat]) {.base.} =
  assert false
...
method update*(self: var SGD, weights: var Matrix[NNFloat],
    gradients: var Matrix[NNFloat]) =
  weights = weights - (gradients * self.lr)

# ==============================================================================

proc newMomentum*(lr: NNFloat = 0.01, momentum: NNFloat = 0.9):
    Momentum {.noSideEffect.} =
  new(result)
  result.lr = lr
  result.momentum = momentum

method update*(self: var Momentum, weights: var Matrix[NNFloat],
    gradients: var Matrix[NNFloat]) =
  if self.v.isNil:
...
method update*(self: var AdaGrad, weights: var Matrix[NNFloat],
    gradients: var Matrix[NNFloat]) =
  if self.h.isNil:
...
proc newAdam*(lr: NNFloat = 0.001, beta1: NNFloat = 0.9, beta2: NNFloat = 0.999):
    Adam {.noSideEffect.} =
  new(result)
  result.lr = lr
  result.beta1 = beta1
...
method update*(self: var Adam, weights: var Matrix[NNFloat],
    gradients: var Matrix[NNFloat]) =
  if self.v.isNil:
  ...
  self.iter.inc
  let lr = self.lr * (1.0 - pow(self.beta2, self.iter.NNFloat)).sqrt /
      (1.0 - pow(self.beta1, self.iter.NNFloat))
